...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'kunai':
				dialogue = CoolUtil.coolTextFile(Paths.txt('kunai/introdialogue'));
			case 'katana':
				dialogue = CoolUtil.coolTextFile(Paths.txt('katana/introdialogue'));
			case 'shuriken':
				dialogue = CoolUtil.coolTextFile(Paths.txt('shuriken/introdialogue'));
		}
		...
		switch(stageCheck)
		{
			case 'ninjacastle':
					{
						curStage = 'ninjacastle';

						var skybox = new FlxSprite(-140, -175).loadGraphic(Paths.image('ninjacastle_sky', 'shinobi'));
						skybox.setGraphicSize(Std.int(skybox.width * 0.5));
						skybox.scrollFactor.set(0.1, 0.1);
						skybox.antialiasing = true;
						add(skybox);

						var ninjabg = new FlxSprite(-150, -130).loadGraphic(Paths.image('ninjacastle_background', 'shinobi'));
						ninjabg.setGraphicSize(Std.int(ninjabg.width * 1));
						ninjabg.scrollFactor.set(0.5, 0.5);
						ninjabg.antialiasing = true;
						ninjabg.updateHitbox();
						add(ninjabg);

						ninjaBop = new FlxSprite(830,250);
						ninjaBop.frames = Paths.getSparrowAtlas('ninjaboppin', 'shinobi');
						ninjaBop.animation.addByPrefix("idle", "ninjaboppin", 24, false);
						ninjaBop.animation.play("idle");
						ninjaBop.setGraphicSize(Std.int(ninjaBop.width * 0.5));
						ninjaBop.scrollFactor.set(0.6, 0.6);
						ninjaBop.antialiasing = true;
						ninjaBop.updateHitbox();
						add(ninjaBop);

						var ninjatrainingarea = new FlxSprite(-200, -125).loadGraphic(Paths.image('ninjacastle_middleground', 'shinobi'));
						ninjatrainingarea.setGraphicSize(Std.int(ninjatrainingarea.width * 1.1));
						ninjatrainingarea.scrollFactor.set(0.7, 0.7);
						ninjatrainingarea.antialiasing = true;
						ninjatrainingarea.updateHitbox();
						add(ninjatrainingarea);

						var ninjastage = new FlxSprite(-300, -170).loadGraphic(Paths.image('ninjacastle_stage', 'shinobi'));
						ninjastage.setGraphicSize(Std.int(ninjastage.width * 1.2));
						ninjastage.scrollFactor.set(0.9, 0.9);
						ninjastage.antialiasing = true;
						ninjastage.updateHitbox();
						add(ninjastage);

						var sakuraplants = new FlxSprite(-300, -100).loadGraphic(Paths.image('ninjacastle_foreground', 'shinobi'));
						sakuraplants.setGraphicSize(Std.int(sakuraplants.width * 1.3));
						sakuraplants.scrollFactor.set(1.3, 1.3);
						sakuraplants.updateHitbox();
						sakuraplants.antialiasing = true;
						add(sakuraplants);
						defaultCamZoom = 0.9;
					}
			case 'stage':
	...
	function IntroDialogue(?dialogueBox:DialogueBox):Void
		{
			switch (StringTools.replace(curSong," ", "-").toLowerCase())
				{
					case 'kunai' | 'katana':
	...
	*/
	...
	override function beatHit()
	{
		...
		if (!dad.animation.curAnim.name.startsWith('attack') && dad.animation.curAnim.finished || !dad.animation.curAnim.name.startsWith('sing') && !dad.animation.curAnim.name.startsWith('attack'))
		{
			dad.dance();
		}
